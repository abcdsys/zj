    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            try {
                ...
                do {
                    var conti = false
                    await tigernina_collectAtuoScore()
                    res = await tigernina_getTaskDetail()

                    for (var p = 0; p < res.lotteryTaskVos[0].badgeAwardVos.length; p++) {
                        if (res.lotteryTaskVos[0].badgeAwardVos[p].status == 3) {
                            await tigernina_getBadgeAward(res.lotteryTaskVos[0].badgeAwardVos[p].awardToken)
                        }
                        ...
                        switch (task.taskType) {
                            ...
                            case 26:
                                ...
                                for (var o = 0; o < tmp.length; o++) {
                                    ...
                                    if (tmp[o].status == 1) {
                                        conti = true
                                        await tigernina_collectScore(tmp[o].taskToken, task.taskId)
                                    }
                                ...
                                break
                            case 2:
                                r = await tigernina_getFeedDetail(task.taskId)
                                var t = 0;
                                for (var o = 0; o < r.productInfoVos.length; o++) {
                                    if (r.productInfoVos[o].status == 1) {
                                        conti = true
                                        await tigernina_collectScore(r.productInfoVos[o].taskToken, task.taskId)
                                        t++
                                        if (t >= 5) break
                                    }
                                ...
                                break
                            case 5:
                                r = await tigernina_getFeedDetail2(task.taskId)
                                var t = 0;
                                for (var o = 0; o < r.browseShopVo.length; o++) {
                                    if (r.browseShopVo[o].status == 1) {
                                        conti = true
                                        await tigernina_collectScore(r.browseShopVo[o].taskToken, task.taskId)
                                        t++
                                        if (t >= 5) break
                                    }
                            ...
                            case 21:
                                for (var o = 0; o < task.brandMemberVos.length; o++) {
                                    if (task.brandMemberVos[o].status == 1) {
                                        ...
                                        await tigernina_collectScore(task.brandMemberVos[o].taskToken, task.taskId)
                                    }
                ...
                await tigernina_sign()
                do {
                    var ret = await tigernina_raise()
                } while (ret)
...
function get_secretp() {
    ...
    return new Promise((resolve) => {
        $.post(taskPostUrl("tigernina_getHomeData", body), async (err, resp, data) => {
            try {
                if (err) {
...
function tigernina_sign() {
    ...
    return new Promise((resolve) => {
        $.post(taskPostUrl("tigernina_sign", body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function tigernina_raise() {
    ...
    return new Promise((resolve) => {
        $.post(taskPostUrl("tigernina_raise", body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function tigernina_collectAtuoScore() {
    let body = {
        "ss": {
            "extraData": {
                "log": "",
    ...
    return new Promise((resolve) => {
        $.post(taskPostUrl("tigernina_collectAtuoScore", body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function tigernina_getTaskDetail() {
    let body = {};
    return new Promise((resolve) => {
        $.post(taskPostUrl("tigernina_getTaskDetail", body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
/**
* @Created by Mol on 2022/01/10
* @description 好友金币互助
*/

function tigernina_collectScore(taskToken, taskId) {
    let body = {
        "taskId": taskId,
        "taskToken": taskToken,
        "actionType": 1,
    ...
    return new Promise((resolve) => {
        $.post(taskPostUrl("tigernina_collectScore", body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function tigernina_getBadgeAward(taskToken) {
    let body = {
        "awardToken": taskToken
    };

    return new Promise((resolve) => {
        $.post(taskPostUrl("tigernina_getBadgeAward", body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function tigernina_getFeedDetail(taskId) {
    let body = {
        "taskId": taskId.toString()
    };

    return new Promise((resolve) => {
        $.post(taskPostUrl("tigernina_getFeedDetail", body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function tigernina_getFeedDetail2(taskId) {
    let body = {
        "taskId": taskId.toString()
    };
