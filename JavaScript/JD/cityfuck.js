...
let cookiesArr = [], //开辟jdcookie存储数组
    cookie = '';
let UA, UAInfo = {}
let exchangeFlag = true;
//添加ck到变量
if ($.isNode()) { //是node环境
    Object.keys(jdCookieNode).forEach((item) => { //返回一个由一个给定对象的自身可枚举属性组成的数组，数组中属性名的排列顺序和正常循环遍历该对象时返回的顺序一致 。forEach函数第一个参数是必需，为当前值，第二个参数为下角标索引
        cookiesArr.push(jdCookieNode[item]) //向jdCookieNode数组添加cookie变量的ck
    })
    if (process.env.JD_DEBUG && process.env.JD_DEBUG === 'false') console.log = () => {}; //传递变量
} else { //其他环境boxjs
    cookiesArr = [$.getdata('CookieJD'), $.getdata('CookieJD2'), ...jsonParse($.getdata('CookiesJD') || "[]").map(item => item.cookie)].filter(item => !!item); //map() 方法返回一个新数组，数组中的元素为原始数组元素调用函数处理后的值。map() 方法按照原始数组元素顺序依次处理元素。
}
