...
let helpPool = $.isNode() ? (process.env.JD_CITY_HELPPOOL === "false" ? true : false) : ($.getdata('JD_CITY_HELPPOOL') === "false" ? false : true) //是否全部助力助力池开关，默认关闭
...
!(async () => {
    ...
    if (exchangeFlag) {
        ...
        console.log(`脚本不会自动抽奖，建议活动快结束开启，默认关闭(在${month},${day}日自动开启抽奖),如需自动抽奖请设置环境变量  JD_CITY_EXCHANGE 为true`);
    }
...
function city_doTaskByTk(taskId, taskToken, actionType = 0) {
    return new Promise((resolve) => {
        $.post(taskPostUrl("city_doTaskByTk", {
            "taskToken": taskToken,
            "taskId": taskId,
